...
```{mermaid}
:caption: "Raw scraped schema-on-arrival from both Amazon product pages showing the two collected tables and their 1:1 relationship via `product_id`."
:label: fig-pre-joined
graph LR
    classDef table fill:#f8f8f8,stroke:#333,stroke-width:1px,rx:10px,ry:10px,font-size:12px;
    classDef linkText font-size:10px;

    A[product_details<br><br>
      product_id<br>
      title<br>
      list_price<br>
      amazon_price<br>
      percentage_saving<br>
      unit_price<br>
      rating<br>
      review_count<br>
      last_month_order<br>
...
```{mermaid}
:caption: "Normalized schema showing `products` referencing `brands` and `flavors` (1:N)."
:label: fig-3nf
graph LR
    classDef table fill:#f8f8f8,stroke:#333,stroke-width:1px,rx:10px,ry:10px,font-size:12px;
    ...
    %% Relationships
    B -- "1 : N via brand_id" --> P
    F -- "1 : N via flavor_id" --> P
```
...
- **percentage_saving** — Fractional discount from `list_price` to `amazon_price` (e.g., -0.20 for 20% off).  
