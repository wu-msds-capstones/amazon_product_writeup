...
We then turned to bivariate analysis between `product_order` and the set of numerical features. A Pearson correlation matrix was computed to quantify linear associations, excluding identifiers and non-numeric variables. The results are presented as a bar chart in @fig-raw-corr, which displays the sorted correlation coefficients of each feature with the target variable.
...
The initial correlation analysis revealed that raw features alone were insufficient to explain variation in `product_order`. As such, a number of engineered features were constructed to capture domain-specific patterns and interaction effects.

To account for brand influence, we aggregated total `last_month_order` volume at the brand level and merged this back into the dataset as a new column, `brand_popularity`. A log-transformed version, `log_brand_popularity`, handled the skewness inherent in this metric.

Similarly, a feature interaction was developed to capture the interaction between product rating and review volume. Specifically, this feature multiplies stars by `review_count`, approximating user engagement in a nonlinear fashion.

To incorporate relative performance within each product’s category, we generated rank-based features such as `category_rank_price` and `category_rank_stars`, which respectively rank a product’s price and rating compared to its category peers. Another category-specific feature, `last_month_rank`, ranked products by their `last_month_order` within the same category to approximate recent consumer demand.

Due to a heavy skew for many products within the data set, we will apply a `log1p` (i.e., log(1 + x), for zero-values) transformation to every column that the function, `skew()`, returns values greater than one. This affects `unit_price`, `review_count`, `last_month_order`, `amazon_price`, `list_price`, `stars`, `percentage_saving`, and `interaction`.
